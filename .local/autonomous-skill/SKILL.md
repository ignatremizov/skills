...
description: Use when a user needs a long-running task to span multiple sessions. This skill manages task decomposition, progress tracking, and non-interactive Codex execution with auto-continuation. Trigger phrases include autonomous, long-running task, multi-session, autonomous skill.
---

# Autonomous Skill - Long-Running Task Orchestration

Run long-lived tasks across multiple sessions using a dual-agent pattern (Initializer + Executor) with automatic continuation via Codex non-interactive mode.
