...
# Athena: Supervisor-Led Spec-Driven Development Workflow

An interactive workflow led by Athena that transforms ideas into comprehensive feature specifications, design documents, and actionable implementation plans with clear review gates.
...
**Storage**: Creates files in `.athena/specs/{feature-name}/` directory (kebab-case naming)
...
## Athena Identity & Philosophy

Read `helpers/athena-identity.md` before responding. It defines leadership tone, approval discipline, and language preference.
...
1. **Generate Initial Requirements**
   - Create `.athena/specs/{feature-name}/requirements.md`
...
- Ensure requirements.md exists at `.athena/specs/{feature-name}/requirements.md`
...
Create `.athena/specs/{feature-name}/design.md` with:
...
Create `.athena/specs/{feature-name}/tasks.md` with:
...
- `.athena/specs/{feature-name}/requirements.md`
- `.athena/specs/{feature-name}/design.md`
- `.athena/specs/{feature-name}/tasks.md`
...
```bash
# Check for .athena directory
if [ -d ".athena/specs" ]; then
  # List features
  ls .athena/specs/

  # For specific feature, check phase
  FEATURE="$1"
  if [ -f ".athena/specs/$FEATURE/requirements.md" ]; then
    echo "Requirements exists"
  fi
  if [ -f ".athena/specs/$FEATURE/design.md" ]; then
    echo "Design exists"
  fi
  if [ -f ".athena/specs/$FEATURE/tasks.md" ]; then
...
Athena provides a structured, iterative approach to feature development:
