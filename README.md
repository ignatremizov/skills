...
All three GPT-5.6 models apply long-context pricing to requests above 272K input tokens: 2x input and 1.5x output pricing applies to the full request. The controlled 372K window can cross that threshold, so use long contexts deliberately and compact before the threshold when the retained history is not worth the added cost. Direct API access may support a larger window, but this OAuth-oriented catalog does not advertise it because delegated sessions configured for 1M repeatedly reached the effective 372K limit. Verify current limits and pricing against the live [GPT-5.6 model guidance](https://developers.openai.com/api/docs/guides/model-guidance?model=gpt-5.6).
