...
- `openai/gpt-5.6/models-upstream.json` as the exact upstream model-catalog snapshot from `openai/codex@ddf04ad`
- `openai/gpt-5.6/astra-upstream.md`, `sol-terra-upstream.md`, and `luna-upstream.md` as extracted upstream model-instruction snapshots

### Model catalog overrides

`openai/gpt-5.6/models-config-controlled.json` is rebuilt from `models-upstream.json` and layers only the deliberate local controls. It carries current upstream runtime metadata, including unified exec, app/plugin and Node REPL capabilities, service tiers, GPT-6 Astra, and hidden Daybreak definitions. It retains the GPT-5.3 Codex and Spark fallback entries, keeps retired GPT-5.4 models visible, preserves the visible `gpt-daybreak-blue` defender alias without cyber-specialty client restrictions, keeps the GPT-5.6 default context window at 372K while advertising an 872K maximum, and clears `tool_mode`, `multi_agent_version`, and `multi_agent_reasoning_effort` selectors for GPT-5.6 and Astra so Codex falls back to local feature config for Code Mode, v1/v2 selection, and delegated reasoning effort instead of model metadata forcing those choices.
...
The controlled Astra prompt retains its upstream authorization, follow-through, active-turn steering, and compaction guidance while using the managed Sol communication and shell conventions. After compaction, the latest user request remains current while earlier input is treated as history unless it identifies unfinished work. Text-only clarification tools may request accessible file or screenshot paths or ask the user to make a remote document available, and subagent commentary acknowledgements are not treated as completed delegated turns. Apps and Plugins guidance is omitted from the static prompt because Codex injects the corresponding developer blocks when those capabilities are available and the catalog flags are enabled. Astra additionally exposes the local `persistent` reasoning level and uses the upstream persistent-mode guidance without its unavailable `update_up_next` instruction.
